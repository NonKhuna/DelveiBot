...
def MongoProcess(iput, UserId, cnt):
    ...
    if intent in ['Flow1-what item', 'Flow1-with item']:
        ...
        if da[1]:
            item = iput['queryResult']['outputContexts'][da[0]
                                                         ]['parameters']['item']
            listItem.append(iput['queryResult']
                            ['fulfillmentMessages'][0]['text']['text'][0])
            if item == 'เงิน':
                tex = ['เงินที่หายรวมกี่บาทครับ?',
                       'เงินหายรวมประมาณกี่บาทครับ', 'กี่บาทครับ?', 'หายกี่บาทครับ']
    ...
    if intent in ['Flow1-where', 'flow1-item-place-money']:
        ...
        if da[1]:
            ...
            if item == 'เงิน':
                place = iput['queryResult']['outputContexts'][da[0]
                                                              ]['parameters']['place']
                number = iput['queryResult']['outputContexts'][da[0]
                                                               ]['parameters']['number']
                check = db[dbFlow2].count(
                ...
                colour = iput['queryResult']['outputContexts'][da[0]
                                                               ]['parameters']['colour']
                place = iput['queryResult']['outputContexts'][da[0]
                                                              ]['parameters']['place']
    ...
    if intent == 'Flow1-where-yes':
        ...
        if da[1]:
            ...
            if item == 'เงิน':
                ...
                tex = ['ผมขอรูปเพิ่มเติมด้วยนะครับ',
                       'ผมขอรูปของที่หายด้วยนะครับ']
    ...
    if intent in ['Flow1-how much']:
        ...
        if 'fin' in iput['queryResult']['outputContexts'][da[0]]['parameters']:
            tex = ['หายอยู่แถวไหนครับ',
                   'จำได้ไหมครับ ว่าหายอยู่แถวไหน?', 'หายอยู่แถวไหนหรอครับ ?']
            listItem.append(random.choice(tex))
            return ["Map", listItem]
        else:
    ...
    if intent == 'Flow1-how much - yes':
        listItem.append(iput['queryResult']
                        ['fulfillmentMessages'][0]['text']['text'][0])
        listItem.append(iput['queryResult']
                        ['fulfillmentMessages'][1]['text']['text'][0])
        return ["Map", listItem]
    ...
    if intent == 'Flow1-colour':
        ...
        if da[1]:
            ...
            if colour == 'ไม่รู้':
                ...
                listItem.append(tex)
            tex = ['ของที่หายเป็นของแบรนด์ไรครับ?',
                   'ของที่หายมีชื่อแบรนด์ไหมครับ']
            listItem.append(random.choice(tex))
            return [-1, listItem]
    ...
    if intent == 'Flow1-tel':
        ...
        if da[1]:
            ...
            Retake = iput['queryResult']['outputContexts'][da[0]
                                                           ]['parameters']['Retake']
            tel = iput['queryResult']['outputContexts'][da[0]
                                                        ]['parameters']['tel']
            CC = checkTel(tel)
            if type(CC) == list:
                return CC
            ...
            if x == 0:
                ...
                if item == 'เงิน':
                    ...
                    db[dbFlow1].insert_one({"_id": Id+'1', "souce": UserId, "tel": CC, "place": place, "number": number,
                                            "retake": Retake, "state": 0, 'item': item, 'url': 'coin', 'text': 'แจ้งเงินหาย ' + str(number) + ' บาท'})
                else:
                    url = iput['queryResult']['outputContexts'][da[0]
                                                                ]['parameters']['url']
                    brand = iput['queryResult']['outputContexts'][da[0]
                                                                  ]['parameters']['brand']
                    colour = iput['queryResult']['outputContexts'][da[0]
                    ...
                    db[dbFlow1].insert_one({"_id": Id+'1', "souce": UserId, "tel": CC, "place": place, 'brand': brand,
                                            'colour': colour, "retake": Retake, "state": 0, 'item': item, 'url': url,  'text': 'แจ้ง' + item + 'หาย'})
                return ["Finish", 1]
            else:
    ...
    if intent == 'Flow1-tel-yes':
        ...
        if da[1]:
            ...
            if item == 'เงิน':
                ...
                db[dbFlow1].insert_one({"_id": Id+'1', "souce": UserId, "tel": CC, "place": place, 'brand': brand,
                                        'colour': colour, "retake": Retake, "state": 0, 'url': url, 'item': item, 'text': 'แจ้ง' + item + 'หาย'})
            return ["Finish", 1]
        else:
            print("ไม่เจอ context")
    ...
    if intent in ['Flow2-what item', 'Flow2-found item']:
        ...
        if da[1]:
            ...
            listItem.append(iput['queryResult']
                            ['fulfillmentMessages'][0]['text']['text'][0])
            if item == 'เงิน':
                tex = ['เก็บได้กี่บาทครับ', 'เก็บได้รวมกี่บาทครับ',
                       'กี่บาทครับ?', 'เจอเงินกี่บาทครับ']
                listItem.append(random.choice(tex))
                print(listItem)
                return [-1, listItem]
            else:
                    ...
                    if colour != 'ไม่รู้' and colour != 'ไม่มี':
                        ...
                        if check2 == 0:
                            ...
                            if brand != 'ไม่รู้' and brand != 'ไม่มี':
                                check3 = db[dbFlow1].count(
                                    {'item': item, 'colour': colour, 'brand': brand, "state": 0})
                                if check3 == 0:
                                    data = data = db[dbFlow1].find(
                                        {'item': item, 'colour': colour, "state": 0})
    ...
    if intent == 'Flow2-colour':
        ...
        if da[1]:
            ...
            if colour == 'ไม่รู้':
                ...
                tex = item+colour+'นะครับ'
                listItem.append(tex)
            tex = ['ของที่เก็บได้ป็นของแบรนด์ไรครับ?',
                   'ของที่เก็บได้มีชื่อแบรนด์ไหมครับ?']
            listItem.append(random.choice(tex))
            return [-1, listItem]
        ...
        if da[1]:
            ...
            if x == 0:
                ...
                if item == 'เงิน':
                    ...
                    db[dbFlow2].insert_one({"_id": Id+'2', "souce": UserId, "tel": CC, "place": place, "number": number,
                                            "retake": Retake, "state": 0, 'item': item, 'url': 'coin', 'text': 'แจ้งเงินหาย ' + str(number) + ' บาท'})
                else:
                    url = iput['queryResult']['outputContexts'][da[0]
                                                                ]['parameters']['url']
                    ...
                    db[dbFlow2].insert_one({"_id": Id+'2', "souce": UserId, "tel": CC, "place": place, 'brand': brand,
                                            'colour': colour, "retake": Retake, "state": 0, 'item': item, 'url': url,  'text': 'แจ้ง' + item + 'หาย'})
                return ["Finish", 1]
            else:
                return "Want_More_infomation?"
    ...
    if intent == 'Flow2-tel-yes':
        ...
        if da[1]:
            ...
            if item == 'เงิน':
                ...
                db[dbFlow2].insert_one({"_id": Id+'2', "souce": UserId, "tel": CC, "place": place, "number": number,
                                        "retake": Retake, "state": 0, 'item': item, 'url': 'coin', 'text': 'แจ้งเก็บเงินได้ ' + str(number) + ' บาท'})
            else:
                url = iput['queryResult']['outputContexts'][da[0]
                                                            ]['parameters']['url']
                brand = iput['queryResult']['outputContexts'][da[0]
                ...
                db[dbFlow2].insert_one({"_id": Id+'2', "souce": UserId, "tel": CC, "place": place, 'brand': brand, 'colour': colour,
                                        "retake": Retake, "state": 0, 'url': url, 'item': item, 'text': 'แจ้งเก็บ' + item + 'ได้'})
            return ["Finish", 1]
        else:
            print("ไม่เจอ context")
    ...
    if intent == 'Flow1-how much - no':
        ...
        if item in ['เงิน']:
            ...
            tex = ['ลองเช็คตัวเองดูยังว่าเราหลงใส่อยู่รึเปล่า เพราะก็เคยมีคนใส่เองแล้วหาไม่เจอนะ',
                   'หลงใส่เองอยู่รึปล่าวว ตามตัวเองไรงี้']
            listItem.append(random.choice(tex))
            return [-1, listItem]
        else:
            tex = ['ได้หลงให้เพื่อนยืมรึเปล่าครับ?',
                   'มีเพื่อนหลงหยิบไปไม่บอกรึเปล่า?']
            listItem.append(random, choice(tex))
    ...
    if intent in ['instructive-state3-item', 'lost_item-what i cando']:
        ...
        if da[1]:
            ...
            if item in ['เงิน']:
                # tex=item+'หายนะครับ'
                listItem.append(
                    'ปกติได้พกกระเป๋าสตางค์ หรือ กระเป๋าเงินไหมครับ')
                return [-2, '<in-instructive-money>', listItem]
            elif item in ['แหวน', 'แว่นตา']:
                tex = ['ลองเช็คตัวเองดูยังว่าเราหลงใส่อยู่รึเปล่า เพราะก็เคยมีคนใส่เองแล้วหาไม่เจอนะ',
                       'หลงใส่เองอยู่รึปล่าวว ตามตัวเองไรงี้']
